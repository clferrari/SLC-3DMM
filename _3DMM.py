...
class _3DMM:
    ...
    def opt_3DMM_fast(self, Weights, Components, Components_res, landmarks3D, id_landmarks_3D, landImage, avgFace,
                      _lambda, rounds, r, C_dist):
        _Weights = Weights
        ...
        # deform shape
        _alpha = self.alphaEstimation(landImage, _proj, Components_res, id_landmarks_3D, _Sa, _Ra, _Ta, Weights,
                                      _lambda)
    ...
    def alphaEstimation(self, landImage, projLandModel, Components_res, id_landmarks_3D, S, R, t, Weights, _lambda):
        ...
        for c in range(Components_res.shape[2]):
            ...
            vertexOnImComp = np.transpose(self.getProjectedVertex(vect, S, R, t))  # fast_3d cancellare sta riga (?)
    ...
    def alphaEstimation_fast_3D(self, landImage, projLandModel, Components_res, id_landmarks_3D, Weights, _lambda):
        ...
        for c in range(Components_res.shape[2]):
            vect = Components_res[id_landmarks_3D, :, c].reshape(landImage.shape[0], 3, order='F')
