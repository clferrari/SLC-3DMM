...
class Matrix_op:
    ...
    def __init__(self, Components, aligned_models_data):
        ...
        if Components is None:
            self.X_aligned_model = np.array(aligned_models_data)
            self.X_after_training = []
            self.X_res = []

    def mean(self):
        self.X_aligned_model = self.X_aligned_model - self.X_aligned_model.mean(axis=1, keepdims=True)

    def normalization(self):
        self.X_aligned_model = norm(self.X_aligned_model, axis=1, ord=1)
